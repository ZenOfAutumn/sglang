...
# ==============================================================================
"""
推理服务器的入口（SRT = SGLang Runtime）。

本文件通过 FastAPI 为推理引擎实现 HTTP API。

【本文件为 http_server.py 的中文注释副本】
代码逻辑与原文件完全一致，仅添加中文注释帮助理解，请勿在生产中直接引用本副本。

整体架构（SRT Server）：
- HTTP Server：基于 FastAPI 的服务器，把请求路由给引擎。
- Engine（引擎）由三个组件构成：
    1. TokenizerManager：对请求做分词，发给 Scheduler；运行在主进程。
    2. Scheduler（子进程）：接收请求、组 batch、前向计算，把输出 token 发给 DetokenizerManager。
    3. DetokenizerManager（子进程）：把输出 token 反分词成文本，回传给 TokenizerManager。
- 进程间通过 ZMQ 做 IPC 通信（各进程使用不同端口）。

本文件主要包含：
- 全局状态 _GlobalState 与多分词器初始化。
- FastAPI 应用、异常处理、各类 HTTP 端点（原生 / OpenAI / Ollama / Anthropic / SageMaker / Vertex 兼容）。
- 服务器预热（warmup）与启动逻辑（launch_server）。
"""
...
# 修复 Python threading 的一个 bug：禁用线程退出时的 atexit 注册回调。
...
# 保存全局状态：HTTP 端点通过 _global_state 访问分词器/模板管理器与调度信息。
@dataclasses.dataclass
class _GlobalState:
    # 分词器管理器（单分词器 / 多分词器路由 / 多分词器 worker 三种形态之一）。
...
async def init_multi_tokenizer() -> ServerArgs:
    """
    多进程分词器模式的初始化函数。
    它从共享内存（shm）读取参数信息，并为当前进程初始化分词器管理器。
    """
    ...
    # 多分词器模式不支持 API key 鉴权。
    assert (
        server_args.api_key is None
    ), "API key is not supported in multi-tokenizer mode"

    # 为当前进程创建一个新的 IPC 名称（每个 worker 独立）。
...
async def lifespan(fast_api_app: FastAPI):
    # FastAPI 生命周期钩子：在服务启动前初始化各类 serving handler、工具服务、预热线程；
    # yield 之后即开始对外提供 HTTP 服务，退出时回收预热线程。
    ...
    # 如启用指标，添加 Prometheus 中间件并开启函数计时。
    if server_args.enable_metrics:
        add_prometheus_middleware(app)
        enable_func_timer()

    # 初始化链路追踪（tracing），并按 PD 角色给线程打标签。
    ...
    # 初始化各类 OpenAI 兼容的 serving handler（补全/聊天/嵌入/分类/打分/重排/分词等）。
    ...
    # yield 之后 FastAPI 开始对外服务；退出时 join 预热线程。
    try:
        yield
    finally:
        warmup_thread.join()
...
# 创建 FastAPI 应用：挂载上面的 lifespan，并根据环境变量决定是否暴露 OpenAPI 文档。
app = FastAPI(
    lifespan=lifespan,
...
##### 原生 API 端点（SGLang 自有接口，非 OpenAI 兼容） #####
...
async def health_generate(request: Request) -> Response:
    """
    通过发送一个生成 1 个 token 的特殊请求来检查推理服务器健康状态。

    若服务器正在处理其他任务，该请求会被忽略，因此零开销。
    若服务器空闲，该请求会被执行，从而判断服务器是否健康。
    """
...
async def weight_version():
    """获取当前权重版本（已废弃，返回 404，请改用 /model_info）。"""
    raise HTTPException(
        status_code=404,
...
async def server_info():
    """获取服务器信息（含 server_args、调度器信息、各 DP 内部状态、版本号）。"""
    # 返回每个 DP（数据并行）的内部状态。
...
async def set_internal_state(obj: SetInternalStateReq, request: Request):
    # 运行时修改内部状态（如 pp_max_micro_batch_size 等），需管理员权限（可选）。
...
if os.environ.get("DUMPER_SERVER_PORT") == "reuse":
    ...
    async def _dumper_control_handler(method: str, request: Request):
        ...
        # 调用调试转储（dumper）控制接口，收集各 worker 结果。
...
# FastAPI 会隐式把请求体的 JSON 转换为 dataclass 对象（obj）。
@app.api_route(
    "/generate",
    methods=["POST", "PUT"],
...
async def generate_request(obj: GenerateReqInput, request: Request):
    """处理生成请求（SGLang 原生接口）。支持流式（SSE）与非流式两种返回。"""
    if obj.stream:
        # 流式：以 text/event-stream 逐块返回，并挂载请求中止任务。
...
async def flush_cache(timeout: float = Query(0.0, ge=0.0)):
    """清空基数（radix）缓存。存在运行中/等待中请求时不会执行。"""
...
async def freeze_gc_async():
    """冻结垃圾回收（把当前对象移入永久代，减少 GC 扫描开销）。详见 engine.freeze_gc。"""
...
async def update_weights_from_disk(obj: UpdateWeightFromDiskReqInput, request: Request):
    """从磁盘原地更新权重，无需重启服务器（返回是否成功及被暂停请求数）。"""
...
async def release_memory_occupation(
    ...
    """临时释放 GPU 显存占用（用于训推一体/权重同步等场景让出显存）。"""
    try:
...
async def resume_memory_occupation(
    ...
    """恢复 GPU 显存占用（与 release_memory_occupation 配对使用）。"""
    try:
...
async def abort_request(obj: AbortReq, request: Request):
    """中止请求（按 rid 中止单个，或 abort_all 中止全部）。"""
    try:
...
async def parse_function_call_request(obj: ParseFunctionCallReq, request: Request):
    """原生 API 端点：从一段文本中解析函数调用（function call）。"""
    # 1) 根据请求体初始化解析器。
...
async def separate_reasoning_request(obj: SeparateReasoningReqInput, request: Request):
    """原生 API 端点：从一段文本中分离出推理（reasoning）部分与正文。"""
    # 1) 根据请求体初始化推理解析器。
...
##### OpenAI 兼容 API 端点（/v1/* 系列） #####


@app.post("/v1/completions", dependencies=[Depends(validate_json_request)])
async def openai_v1_completions(request: CompletionRequest, raw_request: Request):
    """OpenAI 兼容的文本补全端点（/v1/completions）。"""
...
async def openai_v1_chat_completions(
    ...
    """OpenAI 兼容的聊天补全端点（/v1/chat/completions）。"""
...
async def openai_v1_audio_transcriptions(
    ...
    """OpenAI 兼容的音频转录（transcription）端点。"""
    # 仅支持 json / text / verbose_json 三种响应格式。
...
async def available_models():
    """列出可用模型（含基座模型与已加载的 LoRA adapter）。OpenAI 兼容端点。"""
...
async def v1_responses_request(request: dict, raw_request: Request):
    """Responses API 端点（支持 reasoning），可返回流式响应。"""
...
##### Anthropic 兼容 API 端点（/v1/messages） #####
...
async def sagemaker_chat_completions(
    ...
    """SageMaker 推理端点（复用 OpenAI 兼容的聊天补全逻辑）。"""
    return await raw_request.app.state.openai_serving_chat.handle_request(
        request, raw_request
    )
...
async def vertex_generate(vertex_req: VertexGenerateReqInput, raw_request: Request):
    # Vertex AI 兼容端点：把 instances 转换为 GenerateReqInput 后调用原生 /generate。
...
def _execute_server_warmup(server_args: ServerArgs):
    # 服务器预热：先轮询 /model_info 等待服务启动，再发送一个真实推理请求触发 JIT/编译/预缓存。
    headers = {}
    url = server_args.url()
...
def _wait_and_warmup(
    ...
    # 预热线程的主体：（可选）等待权重就绪 → 执行预热 → 标记服务就绪 → 执行启动回调。
...
def _wait_weights_ready():
    """在指定超时时间内等待权重就绪（用于 checkpoint-engine 场景）。"""
...
def _setup_and_run_http_server(
    ...
    """设置全局状态、配置中间件并运行 uvicorn。

    由 launch_server 在子进程启动完毕后调用。
    区分单分词器模式与多分词器模式：
    - 单分词器：参数通过 app 属性传递，可启用 api key 鉴权。
    - 多分词器：参数写入共享内存供各 worker 进程读取。
    """
    # 设置全局状态（供所有 HTTP 端点访问）。
    set_global_state(
        _GlobalState(
            tokenizer_manager=tokenizer_manager,
            template_manager=template_manager,
...
def launch_server(
    ...
    SRT 服务器由一个 HTTP 服务器和一个 SRT 引擎构成。

    - HTTP 服务器：一个 FastAPI 服务器，把请求路由给引擎。
    - 引擎由三个组件构成：
        1. TokenizerManager：对请求分词并发给调度器。
        2. Scheduler（子进程）：从 TokenizerManager 接收请求、调度 batch、前向计算，并把输出 token 发给 DetokenizerManager。
        3. DetokenizerManager（子进程）：把输出 token 反分词并把结果回传给 TokenizerManager。

    注意：
    1. HTTP 服务器、引擎、TokenizerManager 都运行在主进程。
    2. 进程间通信通过 ZMQ 库以 IPC 方式完成（每个进程使用不同端口）。
    """
    # 启动子进程（Scheduler / DetokenizerManager 等），并创建 TokenizerManager。
    (
        tokenizer_manager,
        template_manager,
        port_args,
