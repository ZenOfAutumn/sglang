...
**实现**：`src/policies/bucket.rs` — 按模型维护分桶状态，后台线程周期性调整，主要服务 PD（Prefill/Decode 分离）场景。

### 第一性原理降维
Bucket 面向 PD 分离这一特殊拓扑：Prefill 与 Decode 是**两类异构资源**，需分别分桶均衡，而非用同一把尺子度量。它按 `model_id` 维护独立桶，并由后台线程（`bucket_adjust_interval_secs`，默认 5s）周期性地根据双阈值重新平衡：

$$
\text{rebalance} \iff (\text{max} - \text{min}) > \tau_\text{abs} \;\wedge\; \text{max} > \tau_\text{rel} \cdot \text{min}
$$

### 高阶结构化类比
类似**分级/分区调度**——把异构工作负载装入不同容量的桶，各桶独立做均衡，避免异构资源互相干扰。

### 边界与反模式
- **边界**：适用范围窄，主要绑定 PD 场景，不应作为通用 Regular 流量的默认策略。
- **反模式**：在非 PD 的同质场景强行启用，只会引入不必要的分桶维护开销。

### 演进动机
PD 分离把「一次推理」拆成两个阶段后，单一负载度量失效，Bucket 是为「阶段异构性」量身定制的均衡器。
