...
模型权重可在[此处](https://huggingface.co/Qwen/Qwen3-32B)找到
...
推测模型(speculative model)权重可在[此处](https://huggingface.co/Zhihu-ai/Zhi-Create-Qwen3-32B-Eagle3)找到
...
模型权重可在[此处](https://huggingface.co/Qwen/Qwen3-30B-A3B)找到
...
模型权重可在[此处](https://huggingface.co/Qwen/Qwen3-235B-A22B-Instruct-2507)找到
...
#### 在 2 台 Atlas 800I A3 上以 256K 长序列运行 Qwen3-235B-A22B-Instruct-2507(不使用 CP)

此示例使用 **PD 分离**进行长序列推理,并保持**禁用 context parallel**。
...
模型权重可在[此处](https://huggingface.co/Qwen/Qwen3-VL-8B-Instruct)找到
